...
from core.models import Base, Company, BalanceSheet
...
def main():
    ...
    target_table = Base.metadata.tables.get('balance_sheets')
    if target_table is not None:
        target_table.drop(engine, checkfirst=True)
        print("[+] Old 'balance_sheets' table cleared.")
    ...
    sieve.upsert_health(engine, BalanceSheet, table_name, policy, chunk_size=10000, output_dir=f"stock/{table_name}")
