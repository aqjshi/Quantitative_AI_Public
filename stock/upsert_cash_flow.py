...
from core.models import Base, Company, CashFlow
...
def main():
    table_name="cash-flow-statements"
    ...
    target_table = Base.metadata.tables.get('cash_flow')
    if target_table is not None:
        target_table.drop(engine, checkfirst=True)
        print("[+] Old 'cash_flow' table cleared.")
    ...
    sieve.upsert_health(engine, CashFlow, table_name, policy, chunk_size=10000, output_dir=f"stock/{table_name}")
