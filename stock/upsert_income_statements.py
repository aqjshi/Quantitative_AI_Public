...
from core.models import Base, Company, IncomeStatement
...
def main():
    ...
    target_table = Base.metadata.tables.get('income_statements')
    if target_table is not None:
        target_table.drop(engine, checkfirst=True)
        print("[+] Old 'income_statements' table cleared.")
    ...
    sieve.upsert_health(engine, IncomeStatement, table_name, policy, chunk_size=10000, output_dir=f"stock/{table_name}")
