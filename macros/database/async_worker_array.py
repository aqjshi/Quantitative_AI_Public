...
def execute_db_query_array(query, batch_data, info=""):
                ...
                with conn.transaction():
                    with conn.cursor() as cur:
                        ...
                        with cur.copy(copy_cmd) as copy:
                            # If it's already a formatted string or bytes chunk, write it directly!
                            if isinstance(batch_data, (str, bytes)):
                                copy.write(batch_data)
                            else:
...
def async_db_worker_array(task_queue):
    """
    ...
    while True:
        ...
        try:
            ...
            execute_db_query_array(item[1], item[0], item[2])
